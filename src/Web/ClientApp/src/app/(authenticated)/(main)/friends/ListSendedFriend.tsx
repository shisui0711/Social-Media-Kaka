...
const ListSendedFriend = () => {
  ...
  const {
    ...
    queryKey: ["friends-sended"],
    queryFn: async ({pageParam}): Promise<PaginatedListOfUserDto> => client.getSendedFriendWithPagination(pageParam,20),
    initialPageParam: 1,
  ...
  return (
    ...
    >
      {friends.length > 0 && friends.map((friend) => (
        <FriendComponent isSended key={friend.id} user={friend}/>
      ))}
...
export default ListSendedFriend