...
const ListReceivedFriend = () => {
  ...
  const {
    ...
    queryFn: async ({pageParam}): Promise<PaginatedListOfUserDto> => client.getReceivedFriendWithPagination(pageParam,20),
    initialPageParam: 1,
  ...
  return (
    ...
    >
      {friends.length > 0 && friends.map((friend) => (
        <FriendComponent isReceived key={friend.id} user={friend}/>
      ))}
...
export default ListReceivedFriend